...
class GA:
    ...
    def repair_long_contract_selection(self, yijt):
        """确保长协供应商在每个滚动周期至少有一个合格基地被选中。

        贪心解码器只会对 yijt=1 的供应商-基地-周期分配采购量。长协供应商
        的约束是整个滚动周期合计采购量 >= len(tau_list) * l[j]，如果只在
        一个周期被选中，常见结果就是实际采购量 100000 而要求 200000。
        这里仅修复染色体选择位，不改变目标函数、解码器或约束检查。
        """
        for jsp in Jsp:
            if jsp not in J:
                continue
            eligible_bases = [i for i in I if alpha[i][jsp] >= beta[i]]
            if not eligible_bases:
                continue
            for tau in tau_list:
                if sum(yijt[i, jsp, t, tau] for i in eligible_bases) == 0:
                ...
                for i in I:
                    for j in J:
                        if alpha[i][j] < beta[i]:
                            for tau in tau_list:
                                yijt[i][j][t][tau] = 0
                yijt = self.repair_long_contract_selection(yijt)

                with contextlib.redirect_stdout(io.StringIO()):
                    flag, result, x2, lower_bound = solve_procument_plan_greedy(I, beta, W, w0, e0, H, f, D, S, alpha, L,
                                    J, R, Jsp, z, l, u, r, Q, P, cij,
                                    t, K, tau_list, T, yijt)

            if not flag:
                raise RuntimeError(f"初始化种群失败：连续 {max_attempts} 次未生成可行个体，请检查长协供应商、需求和容量约束。")
    ...
    def evaluate(self, individual: Individual):
        ...
        if self.gen < self.max_gen//2:
            with contextlib.redirect_stdout(io.StringIO()):
                individual.feasible, individual.cost, individual.decode, individual.lowerbound = solve_procument_plan_greedy(I, beta, W, w0, e0, H, f, D, S, alpha, L,
                                    J, R, Jsp, z, l, u, r, Q, P, cij,
                                    t, K, tau_list, T, individual.yijt)
        else:
            individual.feasible, individual.cost, individual.decode, individual.lowerbound = solve_procument_plan(
                n, I, beta, W, w0, e0, H, f, D, S, alpha, L,
    ...
    def crossover(self, parent1: Individual, parent2: Individual, pc=0.5) -> tuple[Individual, Individual]:
    ...
    def mutate2(self,individual):
        ...
        ave_supplier_num = max(1, int(total_decisions / (len(tau_list) * len(I))))
    ...
    def run(self):
        ...
        for gen in range(1, self.max_gen):
            ...
            if not (child1.feasible and child2.feasible):
                print(f"Skip generation {gen}: failed to generate feasible children after {max_retry_count} retries.")
                continue
...
if __name__ == '__main__':
    ...
    filename = r'\optimization_history_4x35-mix.xlsx'
    # ------------------
    ga_start_time = time.time()
    ga = GA(pop_size=500, max_gen=5000, mut_prob=0.2,
            max_time=3600, RA_max=0.8, RA_min=0.5)
    best_solution, history = ga.run()
    best_solution.runtime_seconds = time.time() - ga_start_time
    ...
    history_dir = os.path.join(project_root, "outputs", "history")
    os.makedirs(history_dir, exist_ok=True)
    df.to_excel(os.path.join(history_dir, filename.lstrip("\\/")), index=False, engine="openpyxl")
    ...
    standardized = standardize_solution(best_solution, instance_data, "ga_greedy", instance_name)
